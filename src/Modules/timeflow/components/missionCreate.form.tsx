...
const MissionCreate = ({ refetch }: { refetch: () => void }) => {
  const { startTime, endTime, setStartTime, setEndTime } = useMissionStore();
  const { mutate: createMissionTimeFlow } = useTimeflow.useCreateMission();

  const handleSubmit = () => {
    console.log("Start Time:", startTime);
    console.log("End Time:", endTime);

    const payload: MissionPostType = {
      time_user_start: startTime ? moment(startTime).toISOString() : null,
      time_user_end: endTime ? moment(endTime).toISOString() : null,
    };

    createMissionTimeFlow(payload, {
      onSuccess: () => {
        refetch();
        setStartTime(null);
        setEndTime(null);
        toast.success("ماموریت با موفقیت ثبت شد");
      },
